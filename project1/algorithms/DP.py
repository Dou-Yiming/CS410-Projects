...
class DP:
    ...
    def search(self):
            ...
            for k in tqdm(range(len(self.dps))):
                ...
                for i in range(1, dp.shape[0]):
                    for j in range(1, dp.shape[1]):
                        ...
                        if dp[i-1][j-1] + self.alpha(c1, c2) < min(dp[i-1][j] + self.delta, dp[i][j-1] + self.delta):
                            dp[i][j] = dp[i-1][j-1] + self.alpha(c1, c2)
                            query[i][j] = query[i-1][j-1]+c1
                            value[i][j] = value[i-1][j-1]+c2
                        elif dp[i-1][j] + self.delta < min(dp[i-1][j-1] + self.alpha(c1, c2), dp[i][j-1] + self.delta):
                            dp[i][j] = dp[i-1][j] + self.delta
                            query[i][j] = query[i-1][j]+c1
                            value[i][j] = value[i-1][j]+'-'
                        else:
                            dp[i][j] = dp[i][j-1] + self.delta
                            query[i][j] = query[i][j-1]+'-'
                            value[i][j] = value[i][j-1]+c2
                        # print(value[i][j])
                cur_cost = dp[dp.shape[0]-1][dp.shape[1]-1]
                res = {"mode": 'pairwise' if self.mode == 2 else '3-seq',
                       "cost": cur_cost, "query": query[dp.shape[0]-1][dp.shape[1]-1],
                       "value": value[dp.shape[0]-1][dp.shape[1]-1],
                       } if cur_cost < res['cost'] else res
        elif self.mode == 3:
            for l in tqdm(range(len(self.dps))):
                ...
                for i in range(1, dp.shape[1]):
                    for j in range(1, dp.shape[2]):
                        ...
                        cand = [
                            dp[0][i-1][j-1] + self.alpha(c1, c2) + 2 * self.delta,
                ...
                for i in range(1, dp.shape[0]):
                    for j in range(1, dp.shape[2]):
                        ...
                        c2 = self.loader.database[self.pos[l][1]][j-1]
                        cand = [
                            dp[i-1][0][j-1] + self.alpha(c1, c2) + 2 * self.delta,
        ...
        res["run_time"] = "{:.4f} sec".format(self.time_end-self.time_start)
        pprint(res)
