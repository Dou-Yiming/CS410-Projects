...
class Mission:
    def __init__(self, unit_id: str, target_position: Position, target_action: str = "", cityid: int = -1):
        self.target_position: Position = target_position
        self.target_action: str = target_action
        self.unit_id: str = unit_id
        self.delays: int = 0
        self.cityid: int = cityid #if keepcity cityid=cityID>0,else -1
...
class Missions(defaultdict):
    ...
    def cleanup(self, player: Player,
        ...
        for unit_id in list(self.keys()):
            ...
            # if your target no longer have resource, reconsider your mission and your missons is not keepcity
            if (tuple(mission.target_position) not in convolved_collectable_tiles_xy_set) and int(mission.cityid) < 0 :
                del self[unit_id]
                continue
            # if the city you want to keep is alredy died, reconsider your mission
            if (tuple(mission.target_position) not in player_city_tile_xy_set) and int(mission.cityid) >= 0 :
                del self[unit_id]
                continue
